Feature: Visualização de feed de postagens
  ...
  Scenario: Tela de Carregamento do Feed
    Given O usuário "X" está logado e acessa o Twibbles
    When A tela de Feed é carregada e a timeline está vazia
  ...
  Scenario: Exibição das postagens no feed
    Given O usuário está na tela de feed e há postagens disponíveis no feed
    When As postagens são carregadas
    Then O feed exibe os 20 primeiros posts ordenados por data em ordem decrescente, dispostos em uma coluna
  ...
  Scenario: Responsividade do Feed
    Given O usuário está logado e acessa o Twibbles
    ...
    When O usuário acessa o Twibbles em um dispositivo móvel
  ...
  Scenario: Carregar mais postagens antigas com rolagem
    Given O usuário está logado e visualiza os 20 primeiros posts no feed
    And O feed exibe apenas as postagens mais recentes
    ...
    When O feed carrega as próximas 20 postagens antigas
  ...
  Scenario: Carregar mais postagens novas com rolagem para cima
    Given O usuário está logado e visualiza as postagens mais antigas no feed
    When O usuário rola o feed para cima para carregar postagens mais novas
    Then O feed exibe as postagens mais novas, sem limite de 20, carregando todas as postagens mais recentes disponíveis
    And As postagens mais novas são inseridas automaticamente acima das postagens antigas
  ...
  Scenario: Usuário não segue ninguém
    Given O usuário na tela de feed
    And O usuário não segue nenhum outro usuário
  ...
  Scenario: Nenhuma postagem disponível no feed
    Given O usuário está logado e visualiza seu feed de postagens
    And Ninguém dos usuários seguidos postou recentemente
